        ...
        if (sock_pfd.revents & POLLIN){
            ...
            if(bytes_read < 1){
                ...
                for (int i = 0; i < bytes_read; i++) {
                    ...
                    //if nlohmann::json is completed add it to in_queue
                    if(bracket_count == 0){
                        std::string s = sstream.str();
                        l_args.queue_mutex_->lock();
                        l_args.msg_queue_->emplace_back(nlohmann::json::parse(s)); /* TODO handle nlohmann::json fail to parse */
...
int rosbridge_lib::rosbridge_client::connect(int port_arg, std::string address_arg) {
    ...
    //allocate buffers
    //TODO-low  have different buffer sizes as parameter
    ...
    //connect to server
    //TODO-low make this have a timeout
...
int rosbridge_lib::rosbridge_client::advertise(std::string topic, std::string type){
...
int rosbridge_lib::rosbridge_client::unadvertise(std::string topic){
    auto found = std::find(advertised_topics.begin(), advertised_topics.end(), topic);
...
int rosbridge_lib::rosbridge_client::publish(std::string topic, nlohmann::json msg){
    auto found = std::find(advertised_topics.begin(), advertised_topics.end(), topic);
    if(found == advertised_topics.end()){return -1;}
    else{
	nlohmann::json j;
...
int rosbridge_lib::rosbridge_client::subscribe(std::string topic, std::function<void(json&)> callback){
...
int rosbridge_lib::rosbridge_client::unsubscribe(std::string topic){
    auto topic_match = [&topic](std::tuple<string, std::function<void(json&)>> i){ return topic == std::get<0>(i);};
    auto found = std::find_if(subscribed_topics.begin(), subscribed_topics.end(), topic_match);
...
// TODO maybe make this asynchronous with the main thread
int rosbridge_lib::rosbridge_client::send_queue(){
	...
	std::string j_std::string = j.dump();
...
//invoke a subscribed topics callback
int rosbridge_lib::rosbridge_client::handle_msg(std::string topic, nlohmann::json &j){
    auto topic_match = [&topic](std::tuple<string, std::function<void(json&)>> i){ return topic == std::get<0>(i);};
    auto found = std::find_if(subscribed_topics.begin(), subscribed_topics.end(), topic_match);
    if(found == subscribed_topics.end()){return -1;}
    else{
...
int rosbridge_lib::rosbridge_client::spin_once() {
    ...
    //lock message queue and swap out internals, this is kinda neat
	...
	nlohmann::json j = tmp_queue.front();
	tmp_queue.pop_front();
	std::string op = j["op"];
	if (op == "publish") {
...
// TODO-med find faster implemntation with lookup table
